...
# -*- coding: utf-8 -*-
"""
根据 c20 / c22 外表面探测器通量与剂量 CSV，仿照 plot_c9_flux_dose_heatmaps.py 绘制六面热图。

默认读取 build/c20外表面探测器通量与剂量.csv 与 build/c22外表面探测器通量与剂量.csv，
输出至 build/c20_c22/（含与各面 PNG/SVG 及六面汇总图）。

用法:
  python3 plot_c20_c22_flux_dose_heatmaps.py [--out dir] [--no-png]
"""
...
def make_parse_name(tag: str) -> Callable[[str], tuple[str, int, int] | None]:
    """tag 为 'c20' 或 'c22'，匹配 c20_outer_px_0_0 形式。"""
    rx = re.compile(rf"^{re.escape(tag)}_outer_(px|mx|py|my|pz|mz)_(\d+)_(\d+)$")

    def parse_name(name: str):
        m = rx.match(str(name).strip())
        if not m:
            return None
...
def group_face(rows: list[dict], parse_name) -> dict[str, list[dict]]:
...
def build_matrix(face_rows: list[dict], face: str, col: str, parse_name):
    if not face_rows:
...
def try_matplotlib_png(
    ...
    fig.tight_layout()
    fig.savefig(out_dir / f"{file_prefix}_{face}_{fname_part}.png", dpi=dpi)
    plt.close(fig)
    return True
...
def process_tag(
    ...
    """返回 (svg_count, png_count)。"""
    parse_name = make_parse_name(tag)
    rows = load_rows(csv_path)
    if flux_factor != 1.0:
        apply_flux_scale_to_rows(rows, flux_factor)

    by_face = group_face(rows, parse_name)
    svg_n = 0
    png_ok = 0
    ...
    for face in ["px", "mx", "py", "my", "pz", "mz"]:
        ...
        for col, fname_part, zlabel_base, use_log in METRICS:
            ...
            got = build_matrix(fr, face, col, parse_name)
            if got[0] is None:
                continue
            imax, jmax, mat, row_c, col_c = got
            title = f'{info["title"]} — {zlabel}'
            svg_path = out_dir / f"{tag}_{face}_{fname_part}.svg"
            write_svg(
                svg_path,
                title,
                zlabel,
    ...
    if not no_png:
        ...
        for col, fname_part, zlabel_base, use_log in METRICS:
            ...
            for idx, face in enumerate(["px", "mx", "py", "my", "pz", "mz"]):
                ax = axes[idx]
                got = build_matrix(by_face[face], face, col, parse_name)
                if got[0] is None:
            ...
            fig.tight_layout()
            fig.savefig(out_dir / f"{tag}_六面汇总_{fname_part}.png", dpi=dpi)
            plt.close(fig)
            print(f"  [{tag}] 已保存 {tag}_六面汇总_{fname_part}.png")
...
def main() -> None:
    ...
    default_c20 = here / "build" / "c20外表面探测器通量与剂量.csv"
    default_c22 = here / "build" / "c22外表面探测器通量与剂量.csv"
    default_out = here / "build" / "c20_c22"

    ap = argparse.ArgumentParser(description="c20/c22 外表面剂量/通量热图（仿 c9）")
    ap.add_argument("--c20-csv", type=Path, default=default_c20, dest="c20_csv")
    ap.add_argument("--c22-csv", type=Path, default=default_c22, dest="c22_csv")
    ...
    ap.add_argument("--requirements", type=Path, default=None, help="需求参数输出.txt（解析源强）")
    ap.add_argument(
    ...
    if not args.no_flux_scale_by_source:
        ...
        if req_path is None:
            for p in (args.c20_csv.parent / "需求参数输出.txt", args.c22_csv.parent / "需求参数输出.txt"):
                if p.is_file():
                    req_path = p
                    break
        if req_path is None or not req_path.is_file():
            raise SystemExit("通量乘源强需要 需求参数输出.txt（或 --requirements / --no-flux-scale-by-source）")
        flux_factor = parse_source_intensity_req(req_path)
        print(f"通量列已乘以源强 I = {flux_factor:.6g} /s（来自 {req_path.name}）")

    total_svg = total_png = 0
    jobs = [("c20", args.c20_csv), ("c22", args.c22_csv)]
    for tag, csv_path in jobs:
        if not csv_path.is_file():
            print(f"跳过 {tag}: 未找到 {csv_path}")
            continue
        print(f"=== {tag} <- {csv_path.name} ===")
        s, p = process_tag(tag, csv_path, args.out, flux_factor, axis_lo, axis_hi, args.dpi, args.no_png)
        total_svg += s
        total_png += p

    print(f"共生成 {total_svg} 个 SVG -> {args.out}")
    if total_png:
        print(f"另生成单面 PNG 合计 {total_png} 张（若已安装 numpy/matplotlib）。")
    print("完成。")
