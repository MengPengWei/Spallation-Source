...
{ 
  ...
  fShieldDir->SetGuidance("shielding project commands");
  ...
  fDoorOpenCmd = new G4UIcmdWithABool("/shield/doorOpen", this);
  fDoorOpenCmd->SetGuidance("Set door state true=open false=closed");
  fDoorOpenCmd->SetParameterName("doorOpen", false);
  fDoorOpenCmd->AvailableForStates(G4State_PreInit, G4State_Idle);

  fHotcellWallCmd = new G4UIcmdWithADoubleAndUnit("/shield/hotcellWallThickness", this);
  fHotcellWallCmd->SetGuidance("Set hotcell wall thickness.");
  fHotcellWallCmd->SetParameterName("wallThickness", false);
  fHotcellWallCmd->SetRange("wallThickness>0.");
  fHotcellWallCmd->SetUnitCategory("Length");
  fHotcellWallCmd->AvailableForStates(G4State_PreInit, G4State_Idle);
}
...
{    
  ...
  if (command == fDoorOpenCmd) {
    fDetector->SetDoorOpen(fDoorOpenCmd->GetNewBoolValue(newValue));
  }

  if (command == fHotcellWallCmd) {
    fDetector->SetHotcellWallThickness(fHotcellWallCmd->GetNewDoubleValue(newValue));
  }
}
