...
PrimaryGeneratorAction::PrimaryGeneratorAction()
    ...
    , fProtonGun(nullptr)
    , fFusionNeutronGun(nullptr)
{
    fParticleGun = new G4ParticleGun(1);
    if (C_Is_FUSION)
    {
        fFusionNeutronGun = new FusionNeutronGun(fParticleGun);
        fFusionNeutronGun->Configure();
        fBeamRadius = 0.0;
    }
    else if (C_Is_TT&&PG_Is_ProtonGun)
    {
...
{
    if (C_Is_FUSION && fFusionNeutronGun)
    {
        fFusionNeutronGun->GeneratePrimaries(event);
    }
    else if(C_Is_TT&&PG_Is_ProtonGun && fProtonGun)
    {
